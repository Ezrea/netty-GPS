...
public class GPSHeartBeatHandler extends ChannelInboundHandlerAdapter
{
	...
	{
		SZStandardProtocol sp = (SZStandardProtocol)msg;

		//注册消息
		if(sp.getCmdType()==CmdType.REGISTER_REQ)
		{
			//解析协议里的数据内容——注册信息			
			tRegisterReq registerReq = SZStandardOfDecoderByProtobuf.getTRegisterReq(sp.getGspData().toByteArray());
			System.out.println(registerReq);			
			
//			// 登录成功,把channel存到服务端的map中
//			String clientId = registerReq.getCommonGps().getBusId().toString();
//			GPSClientDetailData clientData = new GPSClientDetailData(clientId, (SocketChannel) ctx.channel(), sp);
//			GPSClientMasterDataMap.addOrReplace(clientId, clientData);
		...
		if (msgString.length() > 0)
		{
			...
			{
				System.out.println("[" + df.format(new Date()) + "] 接收到如下消息：[" + msgString + "]");
			...
			if ("20".equals(GpsUtil.getCommandString(msgString)))
			{
				...
				heartBeat = ctx.executor().scheduleAtFixedRate(new HeartBeatTask(ctx), 30, 30, TimeUnit.SECONDS);
